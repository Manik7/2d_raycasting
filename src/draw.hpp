...
namespace rc
{
    ...
    void draw(Vertex const& from, Vertex const& to, T const& value, GenericImage<T, Height, Width, Padding>& image)
    {
        ...
        const float totalSamplesSquared = length_squared * samplesPerPixel * samplesPerPixel;
        ...
        for(float t = 0; t*t <= totalSamplesSquared; t+=1.0F)
        {
            const int row = round(origin_row + (t*t/(totalSamplesSquared))*delta_row);
            const int col = round(origin_col + (t*t/(totalSamplesSquared))*delta_col);
            if(image.isPixelInBounds(row, col))
            {
                image(row, col) = value;
            }
